...
def normalize_columns(df: DataFrame, important_cols: list[str] | None = None) -> DataFrame:
    ...
    for c in df.columns:
        ...
        if should_normalize:
            ...
            exprs.append(when(cleaned.isNull() | (cleaned == "") | lowered.isin(*NULL_TOKENS), None).otherwise(cleaned).alias(c))
        else:
            exprs.append(col(c))
...
def parse_nullable_double(column_name: str):
    ...
    lowered = lower(raw)
    return when(raw.isNull() | (raw == "") | lowered.isin(*NULL_TOKENS), None).when(raw.rlike(NUMERIC_PATTERN), raw.cast("double")).otherwise(None)


def normalize_and_dedup(df: DataFrame, label: str, important_cols: list[str], key_candidates: list[str], num_partitions: int) -> DataFrame:
...
def update_manifest_status(spark: SparkSession, pg_url: str, jdbc_props: dict, ids: list[int], batch_id: str, status: int, error_message: str | None = None) -> None:
    if not ids:
        return
    err_sql = "NULL"
    if error_message:
...
def claim_manifest_batch(spark: SparkSession, pg_url: str, jdbc_props: dict, job_name: str, batch_id: str, batch_size: int) -> DataFrame:
    pending_query = f"""(
        SELECT id
    ...
    pending_ids = [int(r["id"]) for r in read_jdbc_table(spark, pending_query, pg_url, jdbc_props).collect()]
    if not pending_ids:
...
def read_dim_call_type_subset(spark: SparkSession, pg_url: str, jdbc_props: dict, call_type_codes: list[str]) -> DataFrame:
...
def read_manifest_csv_with_schema(spark: SparkSession, paths: list[str], schema: StructType) -> DataFrame:
    if not paths:
    ...
    df_raw = (
        spark.read.option("header", "true").option("inferSchema", "false").option("enforceSchema", "false").option("mode", "PERMISSIVE").csv(paths)
    )
    projected_cols = [col(c).cast(schema_map[c]).alias(c) if c in df_raw.columns else lit(None).cast(schema_map[c]).alias(c) for c in expected_cols]
    ...
    return df_raw.select(*projected_cols).withColumn("_source_file", input_file_name()).withColumn("_ingested_at", current_timestamp())
...
def build_empty_raw_df(spark: SparkSession, important_cols: list[str]) -> DataFrame:
    ...
    fields.extend([StructField("_source_file", StringType(), True), StructField("_ingested_at", TimestampType(), True)])
...
def process_flexi(df_flexi: DataFrame) -> DataFrame:
    ...
    return (
        df_flexi.withColumn("usage_date_tmp", flexi_usage_date)
...
def process_icc(df_icc: DataFrame) -> DataFrame:
    ...
    return (
        df_icc.withColumn("used_duration", col("used_duration").cast("double"))
